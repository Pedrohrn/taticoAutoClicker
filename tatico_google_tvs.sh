...
[Service]
Type=simple
ExecStart=/bin/bash -c "sleep 30 && exec $CHROME_BIN --start-fullscreen --disable-session-crashed-bubble --disable-infobars --noerrdialogs --no-first-run --load-extension=\"$EXTENSIONS_DIR/taticoAutoClicker\" \"$URL_ALVO\""
Restart=always
RestartSec=10
